...
<div class="row mb-15">
    <div class="col-xs-12 col-md-7 mb-1 ml-1">
        ...
        @foreach($action_buttons_list as $key=>$account_button)
            ...
            @endphp
            <a href="{{$button_href}}" class="{{$control_id}}-btn btn btn-sm btn-primary btn-outline faded mr-1 {{$button_class}}" {{$button_data_str}}>
        ...
        @foreach($data_set_group_list as $key=>$group)
            <button data-val="{{$key}}" class="{{$control_id}}-grp btn btn-sm btn-primary btn-outline faded mr-1">{{$key}}</button>
        @endforeach
        @endif
    </div>
    <div class="col-xs-12 col-md-5 mb-1 ml-1">
