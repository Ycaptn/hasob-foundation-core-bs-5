<div class="row mb-3">
    <div class="col-xs-12 col-md-8">
        ...
        <a href="#" class="{{ $control_id }}-btn-filter btn btn-sm btn-primary btn-outline faded me-1">
        ...
        @if ($filter_is_enabled == true)
        <div class="row">
            <span id="txt-{{$control_id}}-filter-settings" class="small text-danger" style="display:inline-block"></span>
        </div>
        @endif
...
<div class="modal fade" id="mdl-{{ $control_id }}-filter-modal" tabindex="-1" role="dialog" aria-modal="true" aria-hidden="true">
    <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">

            <div class="modal-header">
                <h5 id="lbl-{{ $control_id }}-filter-modal-title" class="modal-title">Filter Results</h5>
                ...
                <form class="form-horizontal" id="frm-{{$control_id}}-filter-modal" role="form" method="POST" enctype="multipart/form-data" action="">
                    @if ($filter_is_enabled==true && isset($filter_group_single_select) && count($filter_group_single_select)>0)
                        @foreach($filter_group_single_select as $filter_single_select_group=>$filter_single_select_options)
                            ...
                            <select class="form-select" id="sel-filter-{{ $control_id }}-{{$filter_single_select_options[0]}}" name="sel-filter-{{ $control_id }}-{{$filter_single_select_options[0]}}">
                            ...
                            <div class="row row-cols-1 row-cols-sm-2 row-cols-lg-4 row-cols-xl-4 g-2 ps-2">
                                ...
                                <div class="col">
                                    <input class="" type="checkbox"
                                            value="{{$key}}"
                                            id="cbx-filter-{{ $control_id }}-{{str_replace(",","",$filter_multiple_select_options[0])}}" 
                                            name="cbx-filter-{{ $control_id }}-{{str_replace(",","",$filter_multiple_select_options[0])}}"
                                    /> {{ucwords($value)}}
                                </div>
                                @endforeach
                            </div>
                    ...
                    @if ($filter_is_enabled==true && isset($filter_group_range_select) && count($filter_group_range_select)>0)
                        ...
                        <div class="col-lg-12 mb-2">
                            {{ $filter_range_select_group }} {{$filter_range_select_options[1]}} <span id="lbl-rng-filter-{{ $control_id }}-{{str_replace(",","",$filter_range_select_options[0])}}"></span><br/>
                            <input type="range" class="form-range" value="0"
                                    ...
                                    id="rng-filter-{{ $control_id }}-{{str_replace(",","",$filter_range_select_options[0])}}"
                            />                        
                        </div>
                        @endforeach
                    @endif
            ...
            <div class="modal-footer" id="div-save-mdl-{{ $control_id }}-filter-modal">
                <button type="button" class="btn btn-warning float-left" id="btn-reset-mdl-{{ $control_id }}-filter-modal" value="add">Reset</button>
                <button type="button" class="btn btn-primary" id="btn-save-mdl-{{ $control_id }}-filter-modal" value="add">Apply</button>
            </div>
