...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        function {{$control_id}}_{{str_replace('\\','_',$query_model)}}_display_results(endpoint_url){
            ...
            $(`#{{$control_id}}-${model_name}-div-table-view`).empty();
            $(`#{{$control_id}}-${model_name}-div-table-view`).append("<span class='text-center m-4 p-4'>Loading.....</span>");
            ...
            $.get(final_endpoint_url).done(function( response ) {
                ...
                if (response != null && response.tables_html != null){
                    $(`#{{$control_id}}-${model_name}-div-table-view`).empty();
                    $(`#{{$control_id}}-${model_name}-div-table-view`).append(response.tables_html);
                }
                if (response != null && response.result_count==0){
                    $(`#{{$control_id}}-${model_name}-div-table-view`).empty();
                    $(`#{{$control_id}}-${model_name}-div-table-view`).append("<span class='text-center ma-20 pa-20' style='padding-bottom:100px'>No results found.</span>");
                }
