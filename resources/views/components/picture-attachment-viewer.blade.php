@if ($attachable != null)
    ...
    @if (count($attachments) > 0)
            ...
            <div class="col-xs-7">
                @if ($show_tags == true && $attachable->tags != null && count($attachable->tags) > 0)
                    @foreach ($attachable->tags as $key => $tag)
                        <button data-val="{{ $tag->id }}"
                            class="{{ $tag->id }}-grp btn btn-sm btn-primary btn-outline faded me-2">{{ $tag->name }}</button>
                    @endforeach
                @endif
            </div>
        </div>
                        ...
                        @foreach ($attachments as $key => $attachment)
                            <div class="col-lg-3 col-md-4 col-sm-6 col-sm-12 mb-3">
                                <div class="h-75 d-inline-block text-center">
                                    <a href="#" class="{{ $control_id }}_picture-box-view"
                                        data-val-image-path="{{ route('fc.attachment.show', $attachment->id) }}">
                                        <img src="{{ route('fc.attachment.show', $attachment->id) }}"
                                        ...
                                        <div class="small file-name">
                                            ...
                                            <span class="small">
                                                ...
                                                <span class="small text-primary">- {!! \Carbon\Carbon::parse($attachment->created_at)->diffForHumans() !!}</span> <span>
                                                    <a href="#" data-val="{{ $attachment->id }}"
                                                        class="btn-delete-picture-attachement"><i
                                                            class="fa fa-trash txt-danger"></i></a><span>
                                                    </span>
                                        </div>
                                    </a>
                                </div>
        ...
        <div class="modal fade" id="{{ $control_id }}_picture-viewer-modal" role="dialog"
            aria-labelledby="{{ $control_id }}_picture-viewer-label" aria-hidden="true">
