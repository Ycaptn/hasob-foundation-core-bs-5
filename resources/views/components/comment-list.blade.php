    ...
    @foreach ($commentable->get_comments() as $comment)
        ...
        <div class="d-flex align-items-center mb-3">
			<div class="sl-avatar avatar avatar-sm avatar-circle">
                <a href="javascript:void(0)">
                @if ( $comment->user->profile_image == null )
                    <img width="42" height="42" class="rounded-circle" src="{{ asset('hasob-foundation-core/imgs/bare-profile.png') }}" />
                    @else
                    <img width="42" height="42" class="rounded-circle" src="{{ route('fc.get-profile-picture', $comment->user->id) }}" />
                @endif
                </a>
            </div>
